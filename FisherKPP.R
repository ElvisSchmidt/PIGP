...
#-----------------------Data Preparing-------------------
#Load Data
data <- read.csv("CellData2.csv", header = TRUE)

# Extract columns
y <- data[, 1]
x <- data[, 2]
t <- data[, 3]
y <- 1000*y # normalize y
...
C = as.vector(c(C1_hat,C2_synthetic))
#cat("C:", C)
#cat("MAX m*C: ",max(m %*% C),"\n")
...
#                       init = list(logcap=log(1.77),logD = log(1000*48/1850^2), loglambda=log(48*0.06)),
...
fit <- optimizing(posterior, data=list(X=X_norm, N=N, K=K, K_deriv=K_deriv, y=y,ysynth=C2_synthetic, m=m, mu=0, beta=1 ),
                  init= list(logD=log(1000*48/1900^2),logcap=log(1.7),loglambda=log(48*0.06),C1=C1_hat,C2=C2_hat),iter=30000, verbose=TRUE,as_vector=FALSE,algorithm="LBFGS",
                 tol_param=0,tol_rel_grad=1)
print(fit$par[c("D_scale","lambda_scale","cap","sigma2","dsigma2")])
#print(fit$par[c("p1","p2","p3")])
#samples <-sampling(posterior,data=list(X=X_norm, N=N, K=K, K_deriv=K_deriv, y=y,ysynth=C2_synthetic, m=m, mu=0, beta=1 ),
#                   init= list( list(logD=log(1000),logcap=log(1.7),loglambda=log(0.6),C1=C1_hat,C2=C2_synthetic) ),
#                   iter=1000, verbose=FALSE,chains=1)
#C1_opt <- fit$par$C1
#C2_opt <- fit$par$C2
#print(C1_opt-y)
#print(C2_opt-C2_synthetic)
#
RHS <- c(C2_synthetic, 1000*48/1900^2*C2_synthetic + 0.06*48*y*(1-y/1.77))
